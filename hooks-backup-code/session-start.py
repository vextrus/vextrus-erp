#!/usr/bin/env python3
"""
Session Start Hook - Read-Only Observer Pattern + Monitoring
Claude Code 2.0.19 Compatible

Purpose: Load task context, suggest MCP servers, display status, show monitoring
...
try:
    from dashboard import display_quick_stats
...
def main():
    ...
    if task_name and SESSIONS_DIR.exists():
        ...
        if task_content:
            ...
            # OPTIMIZED: Load metadata only, not full task content
            # (Saves 2000-3000 lines of context per session start)
            #
            # User can read full task with: cat sessions/tasks/{task_name}.md
            # Or use /explore to understand codebase context
            #
            # Previous behavior: Loaded full task content (3000+ lines)
            # New behavior: Show metadata + reference to full file

            context += f"**Task File**: `sessions/tasks/{task_name}.md`\n\n"

            # Extract only key information from task content (first 20 lines after frontmatter)
            task_lines = task_content.split('\n')
            in_frontmatter = False
            post_frontmatter_lines = []
            line_count = 0
            ...
            for line in task_lines:
                ...
                if not in_frontmatter and line_count < 20:
                    post_frontmatter_lines.append(line)
                    line_count += 1

            if post_frontmatter_lines:
                context += "**Task Summary** (first 20 lines):\n```\n"
                context += '\n'.join(post_frontmatter_lines)
                context += "\n```\n\n"
                context += f"📖 Read full task: `cat sessions/tasks/{task_name}.md`\n"
                context += f"🔍 Explore codebase: `/explore services/[service]`\n\n"
    ...
    if MONITORING_AVAILABLE:
        try:
            ...
            monitoring_text = monitoring_output.getvalue()
            if monitoring_text:
                context += "---\n\n"
                context += "**Multi-Agent Monitoring**\n\n"
                context += "```\n"
                context += monitoring_text
                context += "```\n\n"
                context += "View full dashboard: `python .claude/hooks/dashboard.py`\n\n"
